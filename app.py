...
from db.operations import (ensure_tables_exist, save_assessment,
                           get_assessment_history, get_dimension_trends,
                           get_team_statistics, get_team_members,
...
"""
    ...
    st.markdown("---")
    st.markdown(
        f'<h3 style="color: {primary_color}; text-align: center; margin-top: 2rem;">🤖 AI Assistant</h3>',
        unsafe_allow_html=True)
    st.markdown(
        '<p style="text-align: center; color: #9CA3AF; margin-bottom: 1.5rem;">Get personalized insights and ask questions about your assessment results</p>',
        unsafe_allow_html=True)
    ...
    if os.environ.get("OPENAI_API_KEY"):
        ...
        st.info(
            "💡 **AI Assistant is not configured.** To enable AI-powered insights and chat, please configure your OpenAI API key in the environment variables."
        )
...
def main():
    ...
    if not st.session_state.assessment_complete:
        ...
        if not st.session_state.user_info_collected:
            ...
            st.markdown(f'<p style="color: {st.session_state.primary_color};">Please enter your details to begin the assessment</p>', unsafe_allow_html=True)
            st.markdown(f'<p style="color: {st.session_state.primary_color};">First two fields with * are required</p>', unsafe_allow_html=True)
            ...
            is_valid_email = bool(re.match(
                email_pattern, user_email)) if user_email else False
